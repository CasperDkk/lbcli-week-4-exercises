# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
transaction=""

# Get the transaction ID
txn_id=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.txid')

# Get UTXO details
utxo_vout_1=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vout | .[0] | .n')
utxo_vout_1_value=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vout | .[0] | .value')

utxo_vout_2=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vout | .[1] | .n')
utxo_vout_2_value=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vout | .[1] | .value')

# Calculate total input value
total_input=$(echo "$utxo_vout_1_value + $utxo_vout_2_value" | bc)

# Calculate change amount (total input - 0.2 BTC)
change_amount=$(echo "$total_input - 0.20000000" | bc)

# Create a change address 
change_address="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"  

# Create the raw transaction with nLockTime set to 2041 (25 + 2016)
bitcoin-cli -regtest -named createrawtransaction inputs='''[ { "txid": "'$txn_id'", "vout": '$utxo_vout_1', "sequence": 1 }, { "txid": "'$txn_id'", "vout": '$utxo_vout_2', "sequence": 1 } ]''' outputs='''{ "'$recipient'": 0.20000000, "'$change_address'": '$change_amount' }''' locktime=2041